...
generateSimulatedReads(fasta_fn, 
                       ...
                       read_length = 150,
                       window = 50,
                       output_fasta = "GRCm38.150.fasta.gz")

generateSimulatedReads(fasta_fn, 
                       gtf_fn,
                       read_length = 75, 
                       window = 25,
                       output_fasta = "GRCm38.75.fasta.gz")
```
...
map_reads() {
  ...
  samtools sort -@ 11 -m 2G  $prefix"_Aligned.out.bam" > $prefix".sorted.bam"
  samtools index -@ 8  $prefix".sorted.bam"
  rm -f  $prefix"_Aligned.out.bam"
}

map_reads "GRCm38.150.fasta.gz,GRCm38.75.fasta.gz" "../dbases/gencode.vM25.annotation.gtf" "../dbases/star/GRCm38" "GRCm38"
...
rse <- pileup_sites(
    "GRCm38.sorted.bam",
    fafile = "../dbases/GRCm38.primary_assembly.genome.fa",
    param = fp,
    BPPARAM = bpp)
saveRDS(rse, "GRCm38_sites.rds")

pp <- PileupParam(min_base_quality = 0, min_mapq = 255, min_minor_allele_depth = 1)
sbp <- ScanBamP
pileup("GRCm38.sorted.bam", pileupParam = pp)
```
